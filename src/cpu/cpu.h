...
#define NES_OPCODE(name, mode, code, bytes, cycles, cycles_plus, func)   \
  ...
  const std::map<uint8_t, Opcode> kOpcodes = {
    ...
    NES_OPCODE("STA", kAbsoluteX,
               0x9D, 3, 5, false, &Cpu::STA),
    NES_OPCODE("STA", kAbsoluteY,
               0x99, 3, 5, false, &Cpu::STA),
